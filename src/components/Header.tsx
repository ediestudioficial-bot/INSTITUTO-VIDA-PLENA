...
export function Header() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-brand-deep/5 bg-brand-cream/90 backdrop-blur-xl">
      <div className="editorial-shell flex h-20 items-center justify-between gap-8">
        <Link href="/" className="flex items-center gap-3" aria-label="Instituto Vida Plena — início">
          <span className="grid h-9 w-9 place-items-center border border-brand-deep bg-white text-brand-deep font-serif text-lg">VP</span>
        ...
        <nav className="hidden lg:flex items-center gap-7" aria-label="Navegação principal">
          {links.map(([href, label]) => <Link key={href} href={href} className="text-[13px] font-medium tracking-wide text-brand-deep hover:text-brand-wood transition-colors">{label}</Link>)}
        </nav>
        <div className="hidden lg:flex items-center gap-3">
          <Link href="/en" className="px-3 py-3 text-xs font-semibold tracking-widest text-brand-deep">EN</Link>
          <Link href="/participe" className="bg-brand-deep px-6 py-3 text-xs font-semibold uppercase tracking-[.12em] text-white hover:bg-brand-sage transition-colors">Faça parte</Link>
        </div>
      ...
      {open && <div className="lg:hidden border-t border-brand-deep/10 bg-brand-cream px-6 py-8"><nav className="flex flex-col">
